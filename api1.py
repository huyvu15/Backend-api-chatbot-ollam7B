# response tạm được mấy câu hay hay
import os
import pickle
import ollama
...
def add_chunk_to_database(chunk, file_path):
    ...
    print(f"Added chunk: {chunk[:200]}... from file: {file_path}")  # In ra chunk đầu tiên (cắt ngắn để dễ đọc)
...
def retrieve(query, top_n=5):
    ...
    # Lấy 5 chunk đầu tiên của file có độ tương đồng cao nhất
    top_chunks = [chunk for chunk, _, file_path in similarities[:top_n]]  # Lấy top_n chunk
...
def create_instruction_prompt(top_chunks, file_path):
    ...
    # Thêm thông tin về 5 chunk đầu tiên của file
    instruction_prompt += '\n'.join([f' - {chunk} (file: {file_path})' for chunk in top_chunks])
    
    for chunk in top_chunks:
        instruction_prompt += f'\n - {chunk} (file: {file_path})'
        print(f"Chunk being sent to prompt: {chunk}...")
...
# Khi bắt đầu, tải lại VECTOR_DB từ file nếu có
load_vector_db()

# Nếu chưa có dữ liệu, tiến hành xử lý các file PDF và lưu vào VECTOR_DB
pdf_directory = r"D:\Project2\data\test"  # Đường dẫn thư mục chứa file PDF
if not VECTOR_DB:  # Nếu VECTOR_DB trống
    process_pdfs(pdf_directory)
    save_vector_db()  # Lưu lại VECTOR_DB vào file
